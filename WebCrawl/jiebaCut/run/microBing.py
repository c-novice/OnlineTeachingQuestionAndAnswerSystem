...
def cut(rows):
    ...
    for row in rows:
        ...
        try:
            s = requests.session()
            print(row[2])
            response = requests.get(url=row[2], stream=True, headers=headers, timeout=10).content
            print(len(response))

        except Exception as e:
        # except (requests.exceptions.ConnectionError, requests.exceptions.ChunkedEncodingError, requests.exceptions.ConnectTimeout) as err:
            print(row[2])
            continue
        if len(response)>400000:
            response=response[1:400000]
...
def writeToDatabase(List):
    ...
    userName2 = 'root'
    passWord2 = '12345678'
    conn2 = pymysql.connect(host=serverName2, user=userName2, password=passWord2, database="microbing", charset='utf8mb4')
    ...
    for key in List:
        ...
        for row in List[key]:
            try:
                cursor2.execute("INSERT INTO table_" + key + " VALUES (%s,%s, %s)",
