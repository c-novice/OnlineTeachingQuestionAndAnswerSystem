...
def cut(rows):
    ...
    for row in rows:
        ...
        j += 1
        try:
            response = requests.get(url=row[2], headers=headers, stream=True).content
            print(len(response))
...
def writeToDatabase(List):
    ...
    userName2 = 'root'
    passWord2 = '12345678'
    conn2 = pymysql.connect(host=serverName2, user=userName2, password=passWord2, database="csdn2", charset='utf8mb4')
