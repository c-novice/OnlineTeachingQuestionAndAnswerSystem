...
def cut(rows):
    ...
    for row in rows:
        ...
        try:
            print(row[2])
            s = requests.session()
            response = s.get(url=row[2], stream=True, headers=headers, timeout=10).content
            print(response)
...
def writeToDatabase(List):
    ...
    userName2 = 'root'
    passWord2 = '12345678'
    conn2 = pymysql.connect(host=serverName2, user=userName2, password=passWord2, database="jvejin", charset='utf8mb4')
